...
// And in round 2, the third senator can just announce the victory since he is the only guy in the senate
// who can vote.
...
function predictPartyVictory(senate) {
  ...
  //iterate from 0 to n
  for (let i = 0; i < n; i++) {
     // Fill the queues with the indices of R and D
    if (senate[i] === "R") {
      radiantQueue.push(i);
    } else {
      direQueue.push(i);
  ...
  //iterate untill radiant or dire queue does not became empty -> simulating the rounds
  while (radiantQueue.length > 0 && direQueue.length > 0) {
    let radiantIdx = radiantQueue.shift();
    let direIdx = direQueue.shift();
      // The smaller index wins this round
    if (radiantIdx < direIdx) {
         // Radiant bans Dire, Radiant senator moves to the next round
        radiantQueue.push(radiantIdx+n);
    }else{
         // Dire bans Radiant, Dire senator moves to the next round
